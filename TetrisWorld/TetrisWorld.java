...
//checkpoint 13 6/10/2016 11:56 PM - Sound added, code as efficient as ever (i think)
public class TetrisWorld extends JFrame implements KeyListener, ActionListener{
  private TetrisWorldLogic game; //a separate object that belongs to the logic class will handle all the game's backend
  private int everythingLoaded = 0, currentScore = 0, squareCounter = 0, ggNumber = 0;
  private boolean paused = false; //boolean for whether or not the game is paused
  private char currentS = 'w'; //current shift is North, syncs with the game logic
  private GridClass grid; //the grid for all our pieces
  private SideClass sidebar; //our in game side bar, that tells us future pieces, etc...
  ...
  private Timer timer; //the in game timer, which takes care of moving pieces automatically
  private AudioInputStream backgroundMusic, pauseSound, gameOverSound, dropSound, holdSound; //sound streams
  private Clip clip, pauseClip, gameOverClip, dropClip, holdClip; //sound clips
//  private AudioData musicData;
  ...
  private JLabel[] heldPieces = {retrieveJLabel("y"), retrieveJLabel("p"), retrieveJLabel("g"), retrieveJLabel("r"), 
    retrieveJLabel("b"), retrieveJLabel("o"), retrieveJLabel("c")};
  public TetrisWorld(){
    //frame is set and initialized, with a grid layout as that's what Tetris world is!
    game = new TetrisWorldLogic();
    setTitle("Tetris World");
    setSize(850,702);
  ...
  private void updateShift(char shift){
    //updates the current shift object, and puts in arrow icon for the gif
    if (shift != currentS){
      currentS = shift;
      vip[0][0].removeAll();
  ...
  //helper method to return a JLabel that has the icon given an image name
  ...
  private void pause(){
    if (! paused){
      ...
      remove(grid);
      add(pauseScreen, BorderLayout.WEST);
      revalidate();
      repaint();
    }
    else{
  ...
  private void initializeMusic(){
    try{
      //attempt to initialize each an every song
