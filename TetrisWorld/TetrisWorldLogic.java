...
//logic for the tetris world game, the backend is really just a character 2d array, and blocks are represented by their
//colors
public class TetrisWorldLogic{
  private char[][] board; //our game board
  private char[] blockColors = {'y', 'p', 'g', 'r', 'b', 'o', 'c', 'i'}; //the seven different blocks (and invisible one)
  private char currentShift, gameState; //character for shift, starting down. the game state boolean for game over
  private Piece[] next3Pieces; //an array for the upcoming pieces
  private Piece currentPiece, shadow, heldPiece; //important pieces for the game, notice the shadow itself is a piece
  private int score = 0, shifted = 0, squaresJustCleared = 0; //important integers for the game
  private boolean justShifted = false, pieceJustHeld = false, shadowNotCast = true,  //important bools for the game
    justPlacedItem = false, pieceHeld = false, updateSquares = false, nextUpdate = false;
  private Random random = new Random(); //random number generator for future pieces
  //strings that represent the shapes of a piece
  private final String sq = "     ..  ..     ", t = "     .  ...     ", s = "      .. ..     ", 
    st = "    ..   ..     ", l = "     ... .      ", lt = "    ...   .     ", li = "    ....        ";
  //every piece starts at row number 12 and column number 12, has a char layout ^, a number of tiles, and an ID
  ...
  private Piece[] pieces = {sqP, tP, sP, stP, lP, ltP, liP}; //Piece objects for the pieces
  public TetrisWorldLogic(){
    ...
    int previousRNG = random.nextInt(pieces.length);
    ...
    previousRNG = random.nextInt(pieces.length);
  ...
  //weird move algorithm, seeing if a copy can move before making the actual piece move
  ...
  //row is inspected for being completely full, returning true if so
  ...
  //column is inspected for it being completely full, returning true if so
  ...
  //when a square is cleared, all tiles are pushed outwards
  ...
  //holds a piece, updating all internals
  public void hold(){
  ...
  private void moveOn(){
    ...
    int newPieceID = random.nextInt(pieces.length); 
    ...
    while (newPieceID == next3Pieces[1].getID() || newPieceID == next3Pieces[0].getID() || newPieceID == currentPiece.getID())
      newPieceID = random.nextInt(pieces.length); 
